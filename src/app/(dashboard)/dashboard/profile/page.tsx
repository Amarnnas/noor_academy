...
import { Camera, Save, Upload, User, Mail, Shield } from "lucide-react";
...
export default function ProfilePage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      logger.info("Profile updated", { email: session?.user?.email, name });
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2 p-3 rounded-lg bg-[hsl(var(--muted))]/50 text-sm text-[hsl(var(--muted-foreground))]">
                ...
                {((session?.user as { role?: string })?.role || "مستخدم") === "admin" ? "مشرف" :
                 (session?.user as { role?: string })?.role === "teacher" ? "مدرب" : "مستخدم"}
              </div>
            </div>
          </div>
